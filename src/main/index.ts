...
async function createWindow(refresh: () => Promise<OverlaySnapshot>): Promise<BrowserWindow> {
  ...
  // renderer to flush, and proceed when the flush settles (success or a
  // failure already surfaced in the renderer UI) or on a hard deadline.
  ...
  win.on('close', (event) => {
    ...
    let done = false;
    const onFlushSettled = () => {
      if (done) return;
      done = true;
      clearTimeout(deadline);
      ipcMain.removeListener('drafts:flush-settled', onFlushSettled);
      if (!win.isDestroyed()) win.close();
    };
    const deadline = setTimeout(onFlushSettled, 1500);
    ipcMain.once('drafts:flush-settled', onFlushSettled);
